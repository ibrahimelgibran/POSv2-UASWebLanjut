...
function GrandTotal() {
  ...
  $(TotalPriceArr).each(function () {
    TotalValue += parseFloat(
      $(this).text().replace(/,/g, "").replace("IDR", "")
    );
  });
  ...
  if (discount != null) {
    ...
    $("#totalValue").text(
      accounting.formatMoney(f_discount, { symbol: "IDR", format: "%s %v" })
    );
    $("#totalValue1").text(
      accounting.formatMoney(TotalValue, { format: "%v" })
    );
  } else {
    $("#totalValue").text(
      accounting.formatMoney(TotalValue, { symbol: "IDR", format: "%s %v" })
    );
    $("#totalValue1").text(
      accounting.formatMoney(TotalValue, { format: "%v" })
    );
  ...
  swal({
    ...
    if (value == "") {
      ...
      if (isNaN(qtynum)) {
        ...
        $("#tableData").append(
          "<tr class='prd'><td class='barcode text-center'>" +
            barcode +
            "</td><td class='text-center'>" +
            product +
            "</td><td class='price text-center'>" +
            accounting.formatMoney(price, { symbol: "IDR", format: "%s %v" }) +
            "</td><td class='text-center'>" +
            unit +
            "</td><td class='qty text-center'>" +
            value +
            "</td><td class='totalPrice text-center'>" +
            accounting.formatMoney(total, { symbol: "IDR", format: "%s %v" }) +
...
$(document).on("click", ".Enter", function () {
  ...
  if (TotalPriceArr == 0) {
    ...
    $(".price").each(function () {
      price.push($(this).text().replace(/,/g, "").replace("IDR", ""));
    });
