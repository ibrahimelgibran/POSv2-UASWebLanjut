...
$(document).ready(function () {
  ...
  $(document).on("click", "#add_row", function () {
    ...
    html_code +=
      ...
      count +
      '" class="form-control form-control-sm input-sm barcode" placeholder="Barcode"/></td>';
    html_code +=
      '<td><input type="text" pattern="[A-Za-z]+" title="No number on product name" name="product_name" id="product_name' +
      count +
      '" placeholder="Title" class="product_name form-control form-control-sm input-sm"/></td>';
    html_code +=
      '<td><input type="number" name="quantity" min="1" id="quantity' +
      count +
      '" data-srno="' +
      count +
      '" placeholder="Qty"  class="form-control form-control-sm nput-sm quantity" /></td>';
    html_code +=
      '<td><input type="number" name="buy_price" min="0.00" step="0.00" placeholder="Price" id="buy_price' +
      count +
      '" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm buy_price"></td>';
    html_code +=
      '<td><input type="text" name="unit" pattern="[A-Za-z]+" title="No number on unit" id="unit' +
      count +
      '" placeholder="Kilograms" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm unit"></td>';
    html_code +=
      '<td><input type="number" name="tax_rate" min="1" id="tax_rate' +
      count +
      '" placeholder="%" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm tax_rate"></td>';
    html_code +=
      '<td><input type="number" name="min_qty" min="1" id="min_qty' +
      count +
      '" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm min_qty" placeholder="Qty" /></td>';
    html_code +=
      '<td><input type="text" name="sell_price" readonly id="sell_price' +
      count +
      '" placeholder="Price+Tax" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm sell_price number_only"></td>';
    html_code +=
      '<td><input type="text" name="total_amount" readonly id="total_amount' +
      count +
      '" placeholder="Sum" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm total_amount"></td>';
    html_code +=
      '<td><input type="text" name="remarks" id="remarks' +
      count +
      '" placeholder="Remarks" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm remarks"></td>';
    html_code +=
      '<td><input type="text" name="location" id="location' +
      count +
      '" placeholder="Location" data-srno="' +
      count +
      '" class="form-control form-control-sm input-sm location"></td>';
    html_code +=
      '<td><button type="button" name="remove_row" id="' +
      count +
      '" class="btn btn-sm btn-danger btn-xs remove_row"><i class="fas fa-minus-circle"></i></button></td></tr>';
    $("#invoice-item-table").append(html_code);
  });
  $(document).on("click", ".remove_row", function () {
    var row_id = $(this).attr("id");
    var total_product_amount = $("#total_amount" + row_id)
      .val()
      .replace("P", "");
    var final_amount = $("#final_total_amount").text().replace("IDR", "");
    var result_amount =
      parseFloat(final_amount) - parseFloat(total_product_amount);
    if (isNaN(result_amount)) {
      if (total_product_amount == "") {
        var total_product_amount = 0;
        var minus_total =
          parseFloat(final_amount) - parseFloat(total_product_amount);
        $("#total_amount").text("IDR " + minus_total);
      } else {
        $("#final_total_amount").text("IDR 0.00");
      }
    } else {
      $("#final_total_amount").text("IDR " + result_amount);
    }
  ...
  function final_total(count) {
    ...
    for (j = 1; j <= count; j++) {
      ...
      if (quantity > 0) {
        ...
        if (buy_price > 0) {
          total_amount =
            parseFloat(quantity).toFixed(2) * parseFloat(buy_price).toFixed(2);
        ...
        actual_amount = $("#total_amount" + j)
          .val()
          .replace("P ", "");
        final_product_amount =
          parseFloat(final_product_amount) + parseFloat(actual_amount);
      }
    }
    $("#final_total_amount").text("IDR " + final_product_amount);
  }
